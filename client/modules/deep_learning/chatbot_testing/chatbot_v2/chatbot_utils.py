...
def generator(data, vocab_size=30522, max_input=512, max_output=50):
  """
  ...
  while True:
    for mov in data:
      curr_mov += 1
      print(f"Currently at movie number: {curr_mov}")
      chat_history = [101]
      for converse in mov:
        for i, line in enumerate(converse):
          ...
          dec_outputs = pad_sequences(sequences=[phrase], maxlen=max_output,
            padding="post", truncating="post", value=tokenizer.pad_token_id)
